...
public class PanneauPrincipal  extends JPanel {
    ...
    public void inviteTicTacToe(String alias) {
        ...
        if (pc!=null) {
            pc.invitationAJouerRecue();
        }
    }
    public void annuleInviteTicTacToe(String alias) {
        PanneauChatPrive pc = panneauxPrives.get(alias);
        if (pc!=null) {
            pc.invitationAJouerAnnulee();
        }
    }
    ...
    public void setFenetreTicTacToe(String alias, FenetreTicTacToe fenetreTicTacToe) {
        PanneauChatPrive pcp = panneauxPrives.get(alias);
        if (pcp != null)
            pcp.setFenetreJeu(fenetreTicTacToe);
    }

    public FenetreTicTacToe getFenetreTicTacToe() {
        Iterator<PanneauChatPrive> it = panneauxPrives.values().iterator();
        FenetreTicTacToe fen = null;
        while (it.hasNext()) {
            fen = it.next().getFenetreJeu();
            if (fen!=null)
                return fen;
        }
        return null;
